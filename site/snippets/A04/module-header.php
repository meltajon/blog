...
<!-- Google Web Fonts -->
<link rel="stylesheet" href="//fonts.googleapis.com/css?family=Rambla|Source+Sans+Pro:200,300,400,600,200italic,300italic,400italic" />
		...
		<div id="header-wrapper" class="wrapper">
			...
			<div id="header-panel2">
				...
				<div id="header-description">
					<?php if (c::get('is_personal', false) == true): ?>
					I live my life one quarter-pounder at a time. <span class="inline-block">For those five bites or less...I'M FREE.</span>
					<?php else: ?>
			...
			<nav id="main-nav">
				...
				<?php
				if (c::get('is_personal', false) == true):
					$nav_items = $pages->find('home', 'personal', 'friends', 'tech', 'design', 'wisdom');
				else: 
					$nav_items = $pages->find('home', 'tech', 'design', 'inspiration');
				endif;

				if ($page->uid() == 'inspiration' && (c::get('is_personal', false) == false)):
					$active_section = 'inspiration';
				elseif ($page->parent()->uid() == 'inspiration' && (c::get('is_personal', false) == false)):
					$active_section = $page->parent();
				elseif ($page->template() == 'smart-category'):
					$active_section = $page->uid();
				elseif ($page->template() == 'category'):
					$active_section = $page->section();
				elseif ($page->template() == 'default'):
					$active_section = $page->parent()->section();
				else:
					$active_section = 'home';
				endif;
				?>
				<?php foreach($nav_items as $item): ?>
				<a class="main-nav-item<?php e($active_section == $item->uid(), ' main-nav-item_active') ?>" href="<?php echo $item->url() ?>"><?php echo html($item->title()) ?></a>
				<?php endforeach ?>
