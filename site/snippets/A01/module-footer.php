	...
	<div id="categories" class="categories_light">
		<div id="categories-wrapper" class="wrapper">
			...
			<strong class="nav-title"><a class="nav-title-link" href="/tech">Technology</a></strong>
			<ul class="nav-list">
			<?php
			$items = page('tech')->children()->visible();
			
			foreach($items->flip()->limit(5) as $item): ?>
				<li class="nav-item"><a class="nav-link" href="<?php echo $item->url(); ?>" rel="bookmark"><?php echo html($item->title()) ?></a></li>
			<?php endforeach; ?>
			</ul>

			<strong class="nav-title"><a class="nav-title-link" href="/design">Design</a></strong>
			<ul class="nav-list">
			<?php
			$items = page('design')->children()->visible();
			...
			<strong class="nav-title"><a class="nav-title-link" href="/wisdom">Wisdom</a></strong>
			<ul class="nav-list">
			<?php
			$items = page('wisdom')->children()->visible();
			...
			<?php if (c::get('is_personal', false) == true): ?>
			<strong class="nav-title"><a class="nav-title-link" href="/friends">Friends</a></strong>
			<ul class="nav-list">
			<?php foreach(page('friends')->children()->visible()->flip()->limit(5) as $item): ?>
				<li class="nav-item"><a class="nav-link" href="<?php echo $item->url(); ?>" rel="bookmark"><?php echo html($item->title()) ?></a></li>
			<?php endforeach; ?>
			</ul>
			<?php endif; // is_personal ?>
